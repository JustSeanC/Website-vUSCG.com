...
function normalize_key($value): string
{
...
function stat_alias_map(): array
{
    return [
        ...
        'IP' => 'ip_rated',
        'IPRATED' => 'ip_rated',
        'IPRATEDPILOTS' => 'ip_rated',

        // HITRON aliases
        'HITRON' => 'hitron_rated',
        'HITRONRATED' => 'hitron_rated',
        'HITRONPILOTS' => 'hitron_rated',
        'HITRONRATEDPILOTS' => 'hitron_rated',

        // BLACK JACK aliases
        'BLACKJACK' => 'black_jack_rated',
        'BLACKJACKRATED' => 'black_jack_rated',
        'BLACKJACKPILOTS' => 'black_jack_rated',
        'BLACKJACKRATEDPILOTS' => 'black_jack_rated',
        ...
        'MQ9UAS' => 'mq9_rated',
        'MQ9RATED' => 'mq9_rated',
        'MQ9RATEDPILOTS' => 'mq9_rated',
    ];
}

function load_google_sheet_stats(string $url): array
{
    ...
    $labelMap = stat_alias_map();
    $out = [];

    // Strategy 1: row-oriented parsing (label + value on the same row).
    // Prefer the nearest numeric cell to each recognized label.
    foreach ($rows as $cols) {
        $hasNumber = false;
        foreach ($cols as $c) {
            if (numeric_or_null($c) !== null) {
                $hasNumber = true;
                break;
            }
        }
        ...
        for ($i = 0; $i < count($cols); $i++) {
            $label = normalize_key($cols[$i] ?? '');
            if (!isset($labelMap[$label])) {
                continue;
            }

            $targetKey = $labelMap[$label];
            $value = null;

            // 1) First numeric value to the right on the same row.
            for ($j = $i + 1; $j < count($cols); $j++) {
                $candidate = numeric_or_null($cols[$j] ?? null);
                if ($candidate !== null) {
                    $value = $candidate;
                    break;
                }
            ...
            // 2) Fallback: numeric value to the left.
            if ($value === null) {
                for ($j = $i - 1; $j >= 0; $j--) {
                    $candidate = numeric_or_null($cols[$j] ?? null);
                    if ($candidate !== null) {
    ...
    // Strategy 2: table-style parsing (header row + data row).
    // This helps when labels are only in the first row and values are in following rows.
    $maxCols = 0;
    foreach ($rows as $r) {
    ...
    for ($col = 0; $col < $maxCols; $col++) {
        ...
        // Find first recognizable label in this column.
        for ($row = 0; $row < count($rows); $row++) {
            $cell = $rows[$row][$col] ?? '';
            $normalized = normalize_key($cell);
            if (isset($labelMap[$normalized])) {
                $labelKey = $labelMap[$normalized];

                // Find first numeric value below the label.
                for ($scan = $row + 1; $scan < count($rows); $scan++) {
                    $candidate = numeric_or_null($rows[$scan][$col] ?? null);
                    if ($candidate !== null) {
                        $out[$labelKey] = $candidate;
                        break;
                    }
                }
...
function load_phpvms_stats(array $cfg): array
{
    $mode = $cfg['mode'] ?? 'http';

    if ($mode === 'http') {
        $body = http_get_text((string)($cfg['http_url'] ?? ''), $cfg['http_headers'] ?? []);
        if ($body === null) {
            return [];
        }
    ...
    if ($mode === 'mysql') {
        ...
        try {
            ...
            $stmt = $pdo->query((string)$db['query']);
...
function read_cache(array $cacheCfg): ?array
{
...
function write_cache(array $cacheCfg, array $payload): void
{
    if (empty($cacheCfg['enabled'])) {
        return;
    }
...
$stats = $defaultStats;
$sheetStats = load_google_sheet_stats((string)($config['google_sheet_csv_url'] ?? ''));
